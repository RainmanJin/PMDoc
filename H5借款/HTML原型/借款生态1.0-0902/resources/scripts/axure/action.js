$axure.internal(function($ax) {
    ...
    var _checkFireActionGroup = function(id, type, func) {
        ...
        for(i = 0; i < animations.length; i++) {
            animation = animations[i];
            getAnimation(animation.id, animation.type)();
        }
    ...
    var _queueMove = function (eventInfo, elementId, moveInfo) {
        ...
        //moved the calcualtion inside of animation queue, to ensure we get the right current location
        //var oldTarget = eventInfo.targetElement;
        //eventInfo.targetElement = elementId;
        ...
        if ($ax.public.fn.IsLayer($obj(elementId).type)) {
            ...
            for(var i = 0; i < childrenIds.length; i++) {
                (function(childId) {
                    animations.push({id: childId, type: queueTypes.move, func: function() {
                        $ax('#' + childId).moveBy(deltaLoc.x, deltaLoc.y, moveInfo.options);
                    }});
                })(childrenIds[i]);
            }
                        ...
                        (function (id) {
                            ...
                            animations.push({
                                id: id, type: queueTypes.rotate, func: function () {
                                    console.log("2 " + now);
                                    $ax('#' + id).circularMove(degreeDelta, rotateInfo.options, centerPoint.x, centerPoint.y, rotate);
                                }
                            });
                            animations.push({ id: id, type: queueTypes.move, func: function () { } });
    ...
    var _queueRotate = function(degreeValue, elementId, rotateInfo) {
        _addAnimation(elementId, queueTypes.rotate, function () {
            $ax('#' + elementId).rotate(degreeValue, rotateInfo.options.easing, rotateInfo.options.duration, rotateInfo.rotateType == 'location');
        });
    };
                ...
                if ($ax.public.fn.IsLayer(axObject.type)) {
                    ...
                    for (var idIndex = 0; idIndex < childrenIds.length; idIndex++) {
                        ...
                        (function(childId) {
                            ...
                            if(moves) animations.push({ id: childId, type: queueTypes.move, func: function() {} });
                        })(childrenIds[idIndex]);
                    }
                } else {
