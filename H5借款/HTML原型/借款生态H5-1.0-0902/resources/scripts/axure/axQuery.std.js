...
//            var wasShown = $ax.visibility.IsIdVisible(elementId);
    ...
    $ax.public.fn.circularMove = function(degreeChange, options, centerPointLeft, centerPointTop, axShouldFire) {
        var elementIds = this.getElementIds();
        //if(queue === undefined) queue = true;
        if(axShouldFire === undefined) axShouldFire = true;
        ...
        for(var index = 0; index < elementIds.length; index++) {
            ...
            if (obj.generateCompound) {
                _rotateAroundCompound(elementId, { x: centerPointLeft, y: centerPointTop }, degreeChange, options.easing, options.duration, false, obj, axShouldFire);
            } else {
                $ax.move.circularMove(elementId, degreeChange, { x: centerPointLeft, y: centerPointTop }, options.easing, options.duration, true);
                if(axShouldFire) $ax.move.rotate(elementId, degreeChange, options.easing, options.duration, false, true);
    ...
    $ax.public.fn.resize = function(newLocationAndSizeCss, resizeInfo, axShouldFire, moves) {
        ...
        for(var index = 0; index < elementIds.length; index++) {
            ...
            if(!resizeInfo.easing || resizeInfo.easing == 'none') {
                ...
                if(isDynamicPanel) query.animate(newLocationAndSizeCss, { queue: false, duration: resizeInfo.duration, easing: resizeInfo.easing });
                else {
    ...
    var _getLayerChildrenDeep = $ax.public.fn.getLayerChildrenDeep = function (layerId) {
        var deep = [];
        var children = $ax('#' + layerId).getChildren()[0].children;
        for (var index = 0; index < children.length; index++) {
            var childId = children[index];
            if ($ax.public.fn.IsLayer($obj(childId).type)) {
                var recursiveChildren = _getLayerChildrenDeep(childId);
