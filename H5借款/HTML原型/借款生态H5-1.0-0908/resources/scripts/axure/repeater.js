...
$axure.internal(function($ax) {
    ...
    var _refreshRepeater = function(repeaterId, eventInfo, itemsPregen) {
        ...
        if (itemsPregen) {
            ...
            var top = _getAdaptiveProp(propMap, 'paddingTop', viewId);
            var left = _getAdaptiveProp(propMap, 'paddingLeft', viewId);
            var paddingY = top + _getAdaptiveProp(propMap, 'paddingBottom', viewId);
            var paddingX = left + _getAdaptiveProp(propMap, 'paddingRight', viewId);
            ...
            for(var pos = start; pos < end; pos++) {
                ...
                if(wrap != -1 && i % wrap == 0) {
                    if(vertical) {
                        top = propMap.paddingTop;
                        left += xOffset;
                    } else {
                        left = propMap.paddingLeft;
                        top += yOffset;
                    }
...
//        }
        ...
        if(resized) {
            ...
            if(count != 0) {
                ...
                yCount = wrap == -1 ? 1 : Math.ceil(count / wrap);
                xCount = wrap == -1 ? count : count % wrap;
            ...
            var newWidth = offset.width + (xCount - 1) * xOffset + paddingX;
            var newHeight = offset.height + (yCount - 1) * yOffset + paddingY;

            if(newWidth != oldWidth || newHeight != oldHeight) {
                $jobj(repeaterId).css('width', newWidth);
                $jobj(repeaterId).css('height', newHeight);
                $ax.dynamicPanelManager.fitParentPanel(repeaterId);
            }
        }
    ...
    var resetItemSizes = function (repeaterId, itemSize, bounds, ids, vertical, wrap) {
        ...
        for(var i = 0; i + bounds[0] < bounds[1]; i++) {
            var size = { itemId: ids[i + bounds[0]], width: itemSize.width, height: itemSize.height };
            currSizes.push(size);
    ...
    _repeaterManager.setItemSize = function (repeaterId, itemId, width, height) {
        ...
        if(deltaX) _pushItems(repeaterId, itemId, deltaX, false);
        if(deltaY) _pushItems(repeaterId, itemId, deltaY, true);

        return true;
    }

    var _pushItems = _repeaterManager.pushItems = function (repeaterId, itemId, delta, vertical) {
        if(delta == 0) return;
        ...
        if (!wrap && vert != vertical) {
            ...
            $ax.event.raiseSyntheticEvent(_createElementId(repeaterId, itemId), 'onItemResize');
            return;
        }
